...
namespace Blogz
{
    public static class IdGetter
    {
        ...
        public static string GetHighestFileID()
        {
            string CurrentHighestID = "File00000";
            ...
            int Num = Convert.ToInt32(NewNum);
            Num++;
            NewNum = $"File{Num,5:D5}";
            return NewNum;
        }
        public static string GetHighestDeletedImageID()
        {
        ...
        public static string GetHighestDeletedFileID()
        {
            string CurrentHighestID = "DeletedFile00000";
            string Extension = "";
            DirectoryInfo dInfo = new DirectoryInfo(Essentials.Path + "/Data/DeletedFiles/");

            foreach (FileInfo _image in dInfo.GetFiles("*.*"))
            {
                string CurrName = Path.GetFileNameWithoutExtension(_image.Name);
                if (Convert.ToInt32(CurrName.Split('e')[4]) > Convert.ToInt32(CurrentHighestID.Split('e')[4]))
                {
                    CurrentHighestID = CurrName;
                    Extension = Path.GetExtension(_image.FullName);
                }
            }
            int NumStartIndex = 12;
            string NewNum = "";
            for (int i = 12; i < 16; i++)
            {
                int CurrNum = Convert.ToInt32(CurrentHighestID[i].ToString());
            ...
            for (int i = NumStartIndex; i < 16; i++)
            {
                NewNum += CurrentHighestID[i];
            }
            int Num = Convert.ToInt32(NewNum);
            Num++;
            NewNum = $"DeletedFile{Num,5:D5}" + Extension;
            return NewNum;
        }
    }
}
